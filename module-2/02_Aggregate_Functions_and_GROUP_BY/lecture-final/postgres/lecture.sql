...
SELECT (name || ' is in the state of ' || district) AS city_and_state FROM city
WHERE countrycode = (SELECT code FROM country WHERE name = 'United States');

SELECT (district || '''s favorite city is ' || name) AS city_and_state FROM city
WHERE countrycode = (SELECT code FROM country WHERE name = 'United States');
...
SELECT AVG(lifeexpectancy) AS average_life FROM country;
...
ORDER BY countrycode, isofficial;
...
--SELECT continent, round(CAST(AVG(lifeexpectancy) AS numeric), 2) AS "Average Life Expectancy"
SELECT continent, round(AVG(lifeexpectancy)::numeric, 2) AS "Average Life Expectancy"
FROM country
WHERE lifeexpectancy IS NOT NULL
GROUP BY continent
ORDER BY "Average Life Expectancy" DESC;
...
GROUP BY name
HAVING COUNT(*) > 1
ORDER BY repeat_count DESC, name
