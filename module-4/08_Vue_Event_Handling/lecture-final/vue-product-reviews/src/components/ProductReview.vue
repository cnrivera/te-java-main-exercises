<template>
    <div class="main">
        ...
        <div class="well-display">
            <div class="well" @click="currentFilter = 0">
                <span class="amount">{{ averageRating }}</span>
                Average Rating
            </div>

            <div class="well" @click="currentFilter = 1">
                <span class="amount">{{ numberOfOneStarReviews }}</span>
                1 Star Review{{ numberOfOneStarReviews === 1 ? '' : 's' }}
            </div>

            <div class="well" @click="currentFilter = 2">
                <span class="amount">{{ numberOfTwoStarReviews }}</span>
                2 Star Review{{ numberOfTwoStarReviews === 1 ? '' : 's' }}
            </div>

            <div class="well" @click="currentFilter = 3">
                <span class="amount">{{ numberOfThreeStarReviews }}</span>
                3 Star Review{{ numberOfThreeStarReviews === 1 ? '' : 's' }}
            </div>

            <div class="well" @click="currentFilter = 4">
                <span class="amount">{{ numberOfFourStarReviews }}</span>
                4 Star Review{{ numberOfFourStarReviews === 1 ? '' : 's' }}
            </div>

            <div class="well" @click="currentFilter = 5">
        ...
        <form v-if="showForm === true" v-on:submit.prevent="addNewReview">
            ...
            <button :disabled="!reviewIsValid">Submit</button>
...
export default {
    ...
    computed: {
        filteredReviews() {
            return this.reviews.filter(review => review.rating === this.currentFilter || this.currentFilter === 0);
        },
        reviewIsValid() {
