...
export default {
  ...
  computed: {
    ...
    filteredReviews() {
      let partiallyFiltered = this.reviews.filter(review => this.filter === 0 || this.filter === review.rating);
      return partiallyFiltered.filter(r => r.review.toLowerCase().includes(this.filterBy.toLowerCase()));
    },
